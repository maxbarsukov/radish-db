...
defmodule RadishDB.Raft.Persistence.Store do
  ...
  """
  defun new_with_initial_snapshotting(dir :: Path.t(), factor :: number, snapshot :: Snapshot.t()) ::
          t do
    File.mkdir_p!(dir)
    {_, index_first, _, _} = entry_elected = snapshot.last_committed_entry
    # `log_fd` will be filled soon
    %__MODULE__{dir: dir, log_size_written: 0, log_expansion_factor: factor}
  ...
  """
  defun new_with_snapshot_sent_from_leader(
          dir :: Path.t(),
          factor :: number,
          snapshot :: Snapshot.t()
        ) :: t do
  ...
  """
  defun log_compaction_runnable?(%__MODULE__{
          latest_snapshot_metadata: meta,
  ...
  defp write_snapshot(
    ...
    message =
      {:snapshot_created, snapshot_path, term, last_committed_index, byte_size(compressed)}
  ...
  defun list_snapshots_in(dir :: Path.t()) :: [Path.t()] do
    ...
    # newest first
    |> Enum.sort_by(&snapshot_path_to_commit_index/1, &>=/2)
  end
  ...
  defunp find_log_files_with_committed_entries_only(dir :: Path.t(), i_committed :: LogIndex.t()) ::
           [Path.t()] do
  ...
  defun find_log_files_containing_uncommitted_entries(
          dir :: Path.t(),
  ...
  defunp partition_obsolete_and_live_log_files(
    ...
    |> Enum.map(fn
      ...
      # atom is larger than any integers
      [{_, path}, nil] -> {path, :infinity}
    end)
